...
export default class AuthDrawer extends React.Component {
  ...
  authOnChange(checkedList) {
    ...
    if (!isArray(checkedList)) {
      this.setState({
        authUserDefaultValue: checkedList.target ? checkedList.target.value : null,
      });
    } else {
      this.setState({
  ...
  render() {
            ...
            <>
              {roleAuthScopeCode.map((item) => (
                <Checkbox
                  ...
                  // key={item.value}
                  checked={
                    this.isChanged && scopeChangeList.length > 0
                      ? this.enabledFlag
    ...
    const authDataProps = {
      ...
      authorityPurorg: trRoleDataAuthorityPurorg,
      authorityManagement: trRoleDataAuthorityManagement,
      authorityPuragent: trRoleDataAuthorityPuragent,
      fetchTabListLoading,
      // 公司loading
      updateCompanyLoading,
      fetchCompanyLoading,
