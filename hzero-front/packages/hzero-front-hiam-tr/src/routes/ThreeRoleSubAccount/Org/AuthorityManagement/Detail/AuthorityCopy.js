...
@connect(({ trAuthorityManagement, loading }) => ({
  trAuthorityManagement,
  loading: loading.models.trAuthorityManagement,
}))
export default class AuthorityCopy extends React.Component {
  ...
  cancel(record) {
    ...
    if (record.isCreate) {
      ...
      dispatch({
        type: 'trAuthorityManagement/removeNewAdd',
        payload: {},
      });
    } else {
  ...
  edit(record = {}, flag) {
    ...
    dispatch({
      type: 'trAuthorityManagement/editRow',
      payload: {
        data: [
          ...data.slice(0, index),
          {
  ...
  dataSave() {
    ...
    form.validateFields((err, values) => {
      if (!err) {
        ...
        dispatch({
          type: 'trAuthorityManagement/copyAuthority',
          payload: {
            userId,
            copyUserIdList: arr,
          },
