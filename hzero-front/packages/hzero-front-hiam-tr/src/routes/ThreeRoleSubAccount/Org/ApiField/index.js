...
@cacheComponent({ cacheKey: '/hiam/tr-account/org/api/list' })
    ...
    pagePush({
      pathname:
        path.indexOf('/private') === 0
          ? `/private/hiam/tr-account/org/field/${userId}/${record.id}`
          : `/hiam/tr-account/org/field/${userId}/${record.id}`,
    ...
    return (
      <>
        <Header
          backPath={
            path.indexOf('/private') === 0
              ? `/private/hiam/tr-account/org?access_token=${accessToken}`
              : '/hiam/tr-account/org'
          }
...
function mapStateToProps({ trSubAccountOrg, loading }) {
  const { apiDataSource, apiPagination, requestMethod } = trSubAccountOrg;
  return {
    requestMethod,
    dataSource: apiDataSource,
    pagination: apiPagination,
    loading:
      loading.effects['trSubAccountOrg/apiInit'] || loading.effects['trSubAccountOrg/queryApis'],
  };
}
...
function mapDispatchToProps(dispatch) {
  return {
    apiInit(payload) {
      return dispatch({
        type: 'trSubAccountOrg/apiInit',
        payload,
      });
    },
    queryApis(payload) {
      return dispatch({
        type: 'trSubAccountOrg/queryApis',
        payload,
      });
    },
    pagePush(payload) {
