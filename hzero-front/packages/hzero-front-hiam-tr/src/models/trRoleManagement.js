...
import {
  ...
  fetchParentRole,
  fetchSiteParentRole,
} from '../services/trRoleManagementService';
...
export default {
  ...
  effects: {
    ...
    *fetchParentRole(_, { call, put }) {
      let roleValue;
      if (isTenantRoleLevel()) {
        roleValue = yield call(fetchParentRole);
      } else {
        roleValue = yield call(fetchSiteParentRole);
      }
      const res = getResponse(roleValue);
      if (res) {
        yield put({
          type: 'updateState',
          payload: {
