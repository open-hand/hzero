...
import {
  ...
  postCaptcha,
  resetPassword,
} from '../services/trSubAccountOrgService';
import { getPasswordRule } from '../services/commonService';
// import uuid from 'uuid/v4';

export default {
  namespace: 'trSubAccountOrg',
  state: {
    enumMap: {},
    // 可分配角色
    createSubRoles: [],
  ...
  effects: {
    ...
    *openPassword({ payload }, { put, select }) {
      const { validCodeLimitTimeEnd } = yield select(
        (state) => state.trSubAccountOrg.passwordProps
      );
    ...
    *openPhone({ payload }, { put, select }) {
      const { validCodeLimitTimeEnd } = yield select((state) => state.trSubAccountOrg.phoneProps);
      const { userInfo } = payload;
      yield put({
        type: 'updateState',
        payload: {
    ...
    *closePhone(_, { put, select }) {
      const { validCodeLimitTimeEnd, validCodeSendLimitFlag } = yield select(
        (state) => state.trSubAccountOrg.phoneProps
      );
      yield put({
        type: 'updateState',
        payload: {
    ...
    *fetchChangeEnabled({ payload }, { call }) {
      const res = getResponse(yield call(fetchChangeEnabled, payload));
      return res;
    },
    ...
    *postCaptcha({ payload }, { call, put, select }) {
      const passwordProps = yield select((state) => state.trSubAccountOrg.passwordProps);
      const phoneProps = yield select((state) => state.trSubAccountOrg.phoneProps);
      ...
      if (res) {
        ...
        if (captchaField) {
          setSession(`tr-sub-account-org-phone`, res[captchaField] || 0);
        }
      }
      yield put({
        type: 'updateState',
