...
class Rule extends PureComponent {
  ...
  removeSelf = () => {
    ...
    if (confirmOptions && !this.isEmptyCurrentRule()) {
      renderConfirm({ ...confirmOptions, onOk: doRemove, onCancel: null });
    } else {
      doRemove();
    }
  };
  ...
  render() {
    ...
    const del = (
      <div key="rule-header" className="rule--header">
        {!immutableGroupsMode && (
          <Btn type="delRule" onClick={this.removeSelf} label={deleteLabel} config={config} />
        )}
      </div>
    );
  ...
  render() {
    ...
    return (
      <Col className={classname}>
        {config.settings.showLabels && <label>{config.settings.fieldLabel}</label>}
        <Field
          config={config}
...
class OperatorWrapper extends PureComponent {
  render() {
    ...
    const operator = showOperator && (
      ...
      >
        {config.settings.showLabels && <label>{config.settings.operatorLabel}</label>}
        <Operator
          key="operator"
          config={config}
          selectedField={selectedField}
    ...
    const hiddenOperator = showOperatorLabel && (
      ...
      >
        <div className="rule--operator">
          {config.settings.showLabels ? <label>&nbsp;</label> : null}
