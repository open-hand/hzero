...
export default (Builder, CanMoveFn = null) => {
  class SortableContainer extends Component {
    ...
    _isScrollable(node) {
      ...
      return (
        (overflowY === 'scroll' || overflowY === 'auto') && node.scrollHeight > node.offsetHeight
      );
    }
    ...
    handleDrag(dragInfo, e, canMoveFn) {
      ...
      const dragEl = this._getDraggableNodeEl(treeEl);
      const plhEl = this._getPlaceholderNodeEl(treeEl);
      let dragRect; let plhRect; let hovRect; let treeRect;
      if (dragEl && plhEl) {
        ...
        if (!hovCNodeEl) {
          ...
          if (hovEl) {
            ...
            if (trgRect) {
              ...
              else if (dragLev > Math.max(...allLevs)) closestDragLev = Math.max(...allLevs);
              else if (dragLev < Math.min(...allLevs)) closestDragLev = Math.min(...allLevs);
    ...
    canMove(fromII, toII, placement, toParentII, canMoveFn) {
      ...
      const isPend =
        placement == constants.PLACEMENT_PREPEND || placement == constants.PLACEMENT_APPEND;
