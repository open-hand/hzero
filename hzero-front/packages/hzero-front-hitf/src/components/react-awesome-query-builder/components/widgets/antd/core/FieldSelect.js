...
import { Tooltip, Select } from 'hzero-ui';
import {
  BUILT_IN_PLACEMENTS,
...
export default class FieldSelect extends PureComponent {
  ...
  filterOption = (input, option) => {
    ...
    const valueForFilter = keysForFilter
      .map((k) => (typeof dataForFilter.props[k] == 'string' ? dataForFilter.props[k] : ''))
      .join('\0');
  ...
  renderSelectItems(fields) {
    return keys(fields).map((fieldKey) => {
      ...
      if (items) {
        ...
        return (
          <Option key={_path} value={_path} title={altLabel} grouplabel={grouplabel} label={label}>
            {option}
          </Option>
        );
      }
