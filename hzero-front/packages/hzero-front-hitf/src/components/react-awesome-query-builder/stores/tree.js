...
const removeRule = (state, path, config) => {
  ...
  const isEmptyRoot = isEmptyGroup && parentPath.size == 1;
  const canLeaveEmpty =
    isEmptyGroup && (isParentRuleGroup ? true : config.settings.canLeaveEmptyGroup && !isEmptyRoot);
  if (isEmptyGroup) {
 ...
 * @param {bool} not
 */
const setNot = (state, path, not) => state.setIn(expandTreePath(path, 'properties', 'not'), not);
    ...
    map.withMutations((current) => {
      ...
      if (availOps && availOps.length == 1) newOperator = availOps[0];
        ...
        for (const strategy of setOpOnChangeField || []) {
          ...
          else if (strategy == 'default') newOperator = defaultOperator(config, newField, false);
      ...
      const {
        ...
        newValueType,
        newValueError,
      } = getNewValueForFieldOp(config, config, current, newField, newOperator, 'field', true);
      if (showErrorMessage) {
...
const setValue = (state, path, delta, value, valueType, config, __isInternal) => {
  ...
  if (showErrorMessage) {
    ...
    const valueSrcs = Array.from(
      ...
      (_, i) => state.getIn(expandTreePath(path, 'properties', 'valueSrc', `${i }`)) || null
    );
    ...
    switch (action.type) {
      ...
      case constants.SET_DRAG_START:
        return Object.assign(
          ...
          state,
          { ...unset },
          { dragStart: action.dragStart, dragging: action.dragging, mousePos: action.mousePos }
        );
      ...
      case constants.SET_DRAG_PROGRESS:
        return Object.assign(
          ...
          state,
          { ...unset },
          { mousePos: action.mousePos, dragging: action.dragging }
        );
      ...
      case constants.SET_DRAG_END:
        return Object.assign(
          ...
          state,
          { ...unset },
          { ...emptyDrag, tree: checkEmptyGroups(state.tree, config) }
        );
