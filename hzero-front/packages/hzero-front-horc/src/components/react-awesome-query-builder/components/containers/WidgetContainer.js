  ...
  return class WidgetContainer extends PureComponent {
    ...
    _setValue = (isSpecialRange, delta, widgetType, value, __isInternal) => {
      if (isSpecialRange && Array.isArray(value)) {
        ...
        if (oldRange[0] !== value[0]) this.props.setValue(0, value[0], widgetType, __isInternal);
        if (oldRange[1] !== value[1]) this.props.setValue(1, value[1], widgetType, __isInternal);
      } else {
      ...
      return (
        <Widget name={defaultWidget} config={config}>
          {range(0, cardinality).map((delta) => {
            ...
            const sepLabel = settings.showLabels ? <label>&nbsp;</label> : null;
            const sourceLabel = settings.showLabels ? <label>&nbsp;</label> : null;
            const widgetLabel = settings.showLabels ? <label>{valueLabel.label}</label> : null;
...
const WidgetFactory = ({
  ...
  const valueError =
    (immValueError &&
      (isSpecialRange ? [immValueError.get(0), immValueError.get(1)] : immValueError.get(delta))) ||
    null;
    ...
    if (typeof defaultValue === 'boolean') {
      ...
      if (Array.isArray(defaultValue)) {
        return defaultValue
          ...
          .join(', ');
      } else return getTitleInListValues(fieldSettings.listValues, defaultValue) || defaultValue;
    }
    return `${defaultValue}`;
  }
