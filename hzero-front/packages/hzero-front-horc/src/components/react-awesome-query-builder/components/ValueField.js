...
export default class ValueField extends PureComponent {
  ...
  onPropsChanged(nextProps) {
    ...
    const needUpdateItems =
      !this.items ||
      keysForItems.map((k) => nextProps[k] !== prevProps[k]).filter((ch) => ch).length > 0;
    ...
    function _filter(list, path) {
      for (const rightFieldKey in list) {
        ...
        if (!rightFieldConfig) {
          ...
          let canUse =
            rightFieldConfig.type == expectedType && rightFieldFullkey != leftFieldFullkey;
          const fn = canCompareFieldWithField || config.settings.canCompareFieldWithField;
          if (fn) {
            canUse =
              canUse &&
              fn(leftFieldFullkey, leftFieldConfig, rightFieldFullkey, rightFieldConfig, operator);
          }
