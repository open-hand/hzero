...
export default class FuncWidget extends PureComponent {
  ...
  renderFuncSelect = () => {
    ...
    const widgetLabel = showLabels ? <label>{funcLabel}</label> : null;
  ...
  renderArgSep = (argKey, argDefinition, argIndex, { renderSeps }) => {
    ...
    return (
      <Col className="rule--func--arg-sep">{renderSeps ? renderSeps[argIndex - 1] : ', '}</Col>
    );
  };
...
class ArgWidget extends PureComponent {
  ...
  render() {
    return (
      <Widget {...this.props} setValue={this.setValue} setValueSrc={this.setValueSrc} isFuncArg />
    );
  }
}
