...
const jsonLogicFormatItem = (item, config, meta, isRoot) => {
  ...
  if ((type === 'group' || type === 'rule_group') && children && children.size) {
    ...
    if (meta.usedFields.indexOf(fieldName) == -1) meta.usedFields.push(fieldName);

    // format logic
    let formatteOp = operator;
    if (typeof operatorDefinition.jsonLogic === 'string') formatteOp = operatorDefinition.jsonLogic;
    let fn =
      typeof operatorDefinition.jsonLogic === 'function' ? operatorDefinition.jsonLogic : null;
    if (!fn) {
      const rangeOps = ['<', '<=', '>', '>='];
      fn = (field, op, val, opDef, opOpts) => {
        ...
        } else if (cardinality == 1) return { [formatteOp]: [formattedField, formattedValue] };
