...
function validateRule(item, path, itemId, meta, c) {
  ...
  const _wasValid = field && operator && value && !value.find((v, ind) => v === undefined);
...
export const getNewValueForFieldOp = function (
  ...
  for (let i = 0; i < operatorCardinality; i++) {
    ...
    const newValueWidget =
      vs === 'value' ? newWidget : getWidgetForFieldOp(config, newField, newOperator, 'value');
  ...
  if (canReuseValue) {
    for (let i = 0; i < operatorCardinality; i++) {
      ...
      if (!isValidSrc && i > 0 && vSrc == null) {
        isValidSrc = true;
      } // make exception for range widgets (when changing op from '==' to 'between')
  ...
  if (showErrorMessage) {
    if (
      ...
      newValueSrc.toJS().filter((vs) => vs === 'value' || vs == null).length === operatorCardinality
    ) {
