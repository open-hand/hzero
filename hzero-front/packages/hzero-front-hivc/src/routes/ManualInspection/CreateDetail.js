...
export default class CreateDetail extends PureComponent {
  ...
  @Bind()
  handleBack() {
    closeTab(`/hivc/inspection-history`);
  }
    ...
    return (
      <>
        <Header
          title={
            type === 'ocr'
              ? intl.get('hivc.manualInspection.view.title.ocrResult').d('OCR发票查验结果')
              : intl.get('hivc.manualInspection.view.title.result').d('手工发票查验结果')
          }
          backPath={
            type === 'ocr' ? '/hivc/ocr-inspection/create' : '/hivc/manual-inspection/create'
          }
        ...
        <Content>
          <Card
            ...
            title={
              <h3>{title || intl.get('hivc.manualInspection.view.title.line').d('发票详情')}</h3>
            }
          >
