...
export default class AssignSecGrp extends React.Component {
  ...
  onRow(record) {
    ...
    this.setState({ ...this.getInitialState(), currentSecGrp: record }, () => {
      this.handleChangePermissionType('visit');
    });
  }
   ...
   */
  @Bind()
  fetchVisitPermission() {
    const {
      currentSecGrp: { secGrpId },
    } = this.state;
    const { onFetchVisitPermission = e => e } = this.props;
    onFetchVisitPermission(secGrpId).then(res => {
      if (res) {
  ...
  fetchCardPermission() {
    ...
    const { onFetchCardPermission = e => e } = this.props;
   ...
   */
  @Bind()
  handleShield(record, callBack = e => e) {
    const { roleId, onShield = e => e } = this.props;
    const {
      currentSecGrp: { secGrpId },
    } = this.state;
    const payload = { ...record, roleId, secGrpId };
  ...
  render() {
    ...
    const commonColumns = [
      {
        title: intl.get('hiam.roleManagement.model.roleManagement.secGrpName').d('安全组名称'),
        dataIndex: 'secGrpName',
      },
      {
        title: intl.get('hiam.roleManagement.model.roleManagement.secGrpCode').d('安全组代码'),
    ...
    const addModalOptions = {
      columns: [
        ...commonColumns,
        {
          title: intl.get('hiam.roleManagement.model.roleManagement.secGrpSource').d('安全组来源'),
      ...
      title: intl.get('hiam.roleManagement.view.title.secGrp.choose').d('选择安全组'),
      rowKey: 'secGrpId',
      queryName: 'secGrpName',
      queryCode: 'secGrpCode',
      queryNameDesc: intl
        .get('hiam.roleManagement.model.roleManagement.secGrpName')
        .d('安全组名称'),
      queryCodeDesc: intl
        .get('hiam.roleManagement.model.roleManagement.secGrpCode')
        .d('安全组代码'),
          ...
          <Col span={16} style={{ marginTop: '87px', paddingLeft: '24px' }}>
            <div className={styles['secGrp-detail-container']}>
              <div className={styles['secGrp-detail-container-title']}>
                {intl.get('hiam.roleManagement.view.title.secGrp.detail').d('安全组权限详情')}
              </div>
              {!isEmpty(currentSecGrp) && (
                ...
                >
                  <TabPane
                    tab={intl
                      .get('hiam.securityGroup.view.title.tab.visit.permission')
                      .d('访问权限')}
                    key="visit"
                  >
                    <VisitPermissionTab {...visitPermissionProps} />
                  </TabPane>
                  <TabPane
                    tab={intl
