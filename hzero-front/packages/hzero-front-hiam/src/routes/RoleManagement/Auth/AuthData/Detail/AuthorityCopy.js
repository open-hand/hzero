...
@connect(({ roleDataAuthorityManagement, loading }) => ({
  roleDataAuthorityManagement,
  loading: loading.models.roleDataAuthorityManagement,
}))
export default class AuthorityCopy extends PureComponent {
  ...
  //   dispatch({
  //     type: 'roleDataAuthorityManagement/editRow',
  //     payload: {
  //       data: [
  //         ...data.slice(0, index),
  //         {
  ...
  dataSave() {
    ...
    form.validateFields((err, values) => {
      if (!err) {
        ...
        dispatch({
          type: 'roleDataAuthorityManagement/copyAuthority',
          payload: {
            roleId,
            copyRoleIdList: arr,
          },
