    ...
    const backPath =
      path.indexOf('/private') === 0
        ? fromSource
          ...
          : `/private/hiam/role/api/${roleId}?access_token=${accessToken}`
        : fromSource
        ? `/hiam/role-tree/api/${roleId}?fromSource=${fromSource}`
        : `/hiam/role/api/${roleId}`;
...
function mapStateToProps({ roleManagement, loading }) {
  const {
    fieldPermissionDataSource,
    fieldPermissionPagination,
    fieldType,
  ...
  return {
    ...
    loading:
      loading.effects['roleManagement/fieldPermissionInit'] ||
      loading.effects['roleManagement/queryFieldPermissions'],
    saveLoading:
      loading.effects['roleManagement/updateFieldPermission'] ||
      loading.effects['roleManagement/createFieldPermission'],
