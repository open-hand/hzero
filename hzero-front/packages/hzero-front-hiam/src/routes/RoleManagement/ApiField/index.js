...
export default class ApiField extends Component {
  ...
  handleApiFieldPermissionMaintain(record) {
    ...
    let routerConfig = {
      pathname: `/hiam/role/field/${roleId}/${record.id}`,
    };
    if (fromSource) {
      routerConfig = {
    ...
    const backPath =
      path.indexOf('/private') === 0
        ? fromSource
          ...
          : `/private/hiam/role?access_token=${accessToken}`
        : fromSource
        ? '/hiam/role-tree'
        : '/hiam/role';
    return (
...
function mapStateToProps({ roleManagement, loading }) {
  const { apiDataSource, apiPagination, requestMethod } = roleManagement;
  return {
    requestMethod,
    dataSource: apiDataSource,
    pagination: apiPagination,
    loading:
      loading.effects['roleManagement/apiInit'] || loading.effects['roleManagement/queryApis'],
  };
}
