...
@connect(({ loading, subAccount, user }) => ({
  user,
...
export default class SubAccountSite extends React.Component {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'subAccount/queryDimension',
    });
    // 获取公钥
    dispatch({
        ...
        render: (text, record) => {
          ...
          const operators = [
            ...
            },
            {
              ...
              len: 4,
              title: intl.get('hiam.subAccount.view.button.viewEmployee').d('查看员工'),
            },
            {
              key: 'userGroup',
              ele: (
