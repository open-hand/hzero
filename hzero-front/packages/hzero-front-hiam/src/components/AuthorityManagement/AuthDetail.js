...
class AuthDetail extends React.Component {
  ...
  addValueList(addRows) {
    ...
    dispatch({
      type: `${!lodash.isUndefined(roleId) ? 'roleDataAuthority' : 'authority'}${lodash.upperFirst(
        code
  ...
  remove() {
    ...
    const onOk = () => {
      dispatch({
        type: `${
          !lodash.isUndefined(roleId) ? 'roleDataAuthority' : 'authority'
...
function mapStateToProps(state, props) {
  ...
  return {
    ...
    addLoading:
      loading.effects[
      `${!lodash.isUndefined(roleId) ? 'roleDataAuthority' : 'authority'}${lodash.upperFirst(
        code
      )}/addAuthority${lodash.upperFirst(code)}`
      ],
    fetchLoading:
