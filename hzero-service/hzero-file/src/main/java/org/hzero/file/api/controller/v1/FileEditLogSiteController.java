 ...
 */
@Api(tags = FileSwaggerApiConfig.FILE_EDIT_LOG_SITE)
@RestController("fileEditLogSiteController.v1")
@RequestMapping("/v1/file-edit-logs")
public class FileEditLogSiteController extends BaseController {

    private final FileEditLogRepository fileEditLogRepository;

    @Autowired
    public FileEditLogSiteController(FileEditLogRepository fileEditLogRepository) {
    ...
    public ResponseEntity<Page<FileEditLog>> list(Long tenantId, String fileName, String realName, String editType, Date changeDateFrom, Date changeDateTo,
                                                  @ApiIgnore @SortDefault(value = FileEditLog.FIELD_LOG_ID, direction = Sort.Direction.DESC) PageRequest pageRequest) {
        return Results.success(fileEditLogRepository.pageEditLog(pageRequest, tenantId, fileName, realName, editType, changeDateFrom, changeDateTo));
    }
}
