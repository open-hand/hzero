...
public class LovController extends BaseController {
    ...
    @Autowired
    private LovPermissionService lovPermissionService;

    private static final Map<String, Object> EMPTY_OBJ = new HashMap<>();
    private static final String INCLUDE_INVALID_FLAG = "includeInvalidFlag";
    ...
    public ResponseEntity<Page<LovValueDTO>> pageLovValue(@ApiParam("租户ID") @PathVariable("organizationId") Long organizationId,
        ...
        checkLovRole(lovValue == null ? null : lovValue.getLovCode());
    ...
    public ResponseEntity<Map<String, List<LovValueDTO>>> batchQueryLovValue(
            ...
            @RequestParam(required = false) Long tenantId) {
        checkLovPermission(extractLovCodes(queryMap, true));
        if (tenantId != null) {
            organizationId = tenantId;
        }
    ...
    public ResponseEntity<List<LovValueDTO>> queryLovValueTree(
            ...
            @PathVariable("organizationId") Long organizationId,
            @RequestParam(required = false) Long tenantId) {
        checkLovPermission(extractLovCodes(queryMap, false));
        if (tenantId != null) {
            organizationId = tenantId;
        }
    ...
    private void checkLovPermission(String lovCode) {
        checkLovPermission(Collections.singleton(lovCode));
    }

    private void checkLovPermission(Collection<String> lovCodes) {
        if (!this.lovPermissionService.checkPermission(lovCodes)) {
            throwLovPermissionDenied(lovCodes);
        }
    }

    private void checkLovRole(String lovCode) {
        checkLovRole(Collections.singleton(lovCode));
    }

    private void checkLovRole(Collection<String> lovCodes) {
        if (!this.lovPermissionService.checkRole()) {
            throwLovPermissionDenied(lovCodes);
        }
    }

    private void throwLovPermissionDenied(Collection<String> lovCodes) {
        String checkedLovCodes = Optional.ofNullable(lovCodes)
                .orElse(Collections.emptyList())
                .stream()
                .filter(StringUtils::isNotBlank)
                .distinct()
                .collect(Collectors.joining(BaseConstants.Symbol.COMMA));
        throw new CommonException(HpfmMsgCodeConstants.ERROR_LOV_PERMISSION_DENIED, checkedLovCodes);
    }

    private List<String> extractLovCodes(Map<String, String> queryMap, boolean useValues) {
        if (queryMap == null) {
            return Collections.emptyList();
        }
        List<String> lovCodes = new ArrayList<>(queryMap.size());
        queryMap.forEach((key, value) -> {
            if (!LovValue.FIELD_TENANT_ID.equals(key) && !INCLUDE_INVALID_FLAG.equals(key)) {
                lovCodes.add(useValues ? value : key);
            }
        });
        return lovCodes;
    }
