 ...
 */
@Api(tags = PlatformSwaggerApiConfig.DASHBOARD_ROLE_CARD)
@RestController("dashboardRoleCardController.v1")
@RequestMapping("/v1/{organizationId}/dashboard-role-cards")
public class DashboardRoleCardController extends BaseController {
    ...
    @Permission(level = ResourceLevel.ORGANIZATION)
    @PostMapping
    public ResponseEntity<List<DashboardRoleCard>> batchCreateOrUpdateRoleCard(@PathVariable("organizationId") Long tenantId, @RequestBody @Encrypt List<DashboardRoleCard> dashboardRoleCards) {
    ...
    public ResponseEntity<Page<DashboardRoleCard>> getRoleAssignCard(@PathVariable("organizationId") Long tenantId,
                                                                     @Encrypt DashboardRoleCard dashboardRoleCard,
                                                                     @ApiIgnore @SortDefault(value = DashboardRoleCard.FIELD_CARD_ID, direction = Sort.Direction.DESC) PageRequest pageRequest) {
        dashboardRoleCard.setTenantId(tenantId);
