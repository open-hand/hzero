...
import org.hzero.platform.infra.feign.impl.UserDetailRemoteServiceImpl;
 ...
 */
@FeignClient(value = HZeroService.Oauth.NAME, fallback = UserDetailRemoteServiceImpl.class, path = "/oauth/api/user")
public interface UserDetailRemoteService {
    ...
    ResponseEntity storeUserAdditionInfo(@RequestParam("access_token") String accessToken,
                                         ...
                                         @RequestParam(value = "childrenDataHierarchyCodes", required = false) List<String> childrenDataHierarchyCodes);
}
