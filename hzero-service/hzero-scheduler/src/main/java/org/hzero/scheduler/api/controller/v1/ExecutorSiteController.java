 ...
 */
@Api(tags = SchedulerSwaggerApiConfig.EXECUTOR_SITE)
@RestController("executorSiteController.v1")
@RequestMapping("/v1/executors")
public class ExecutorSiteController extends BaseController {

    private final ExecutorService executorService;

    @Autowired
    public ExecutorSiteController(ExecutorService executorService) {
    ...
    public ResponseEntity<Page<Executor>> pageExecutor(@ApiIgnore @SortDefault(value = Executor.FIELD_EXECUTOR_ID, direction = Sort.Direction.DESC) PageRequest pageRequest,
                                                       String executorCode, String executorName, Integer executorType, String status, Long tenantId) {
        return Results.success(executorService.pageExecutor(pageRequest, executorCode, executorName, executorType, status, tenantId));
    }

    @ApiOperation(value = "创建执行器")
    @Permission(level = ResourceLevel.SITE)
    @PostMapping
    public ResponseEntity<Executor> createExecutor(@RequestBody @Encrypt Executor executor) {
    ...
    @Permission(level = ResourceLevel.SITE)
    @PutMapping
    public ResponseEntity<Executor> updateExecutor(@RequestBody @Encrypt Executor executor) {
        validObject(executor);
