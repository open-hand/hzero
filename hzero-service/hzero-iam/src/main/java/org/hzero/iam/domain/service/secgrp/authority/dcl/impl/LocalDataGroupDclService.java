 ...
 */
@Component
public class LocalDataGroupDclService extends AbstractLocalDclService {
    @Override
    public SecGrpDclDTO querySecGrpDclAssignableAuthority(@Nonnull SecGrpDclQueryDTO queryDTO, @Nonnull PageRequest pageRequest) {
        ...
        boolean selfDim = this.secGrpDclDimRepository.isSelfManagementDim(queryDTO.getSecGrpId(),
                queryDTO.getRoleId(), Constants.DocLocalAuthorityTypeCode.DATA_GROUP);
        if (selfDim || this.isSuperAdmin(queryDTO.getRoleId())) {
            SecGrpDclDTO result = new SecGrpDclDTO();
            Page<SecGrpDclLine> page = PageHelper.doPageAndSort(pageRequest, () -> secGrpDclLineMapper.selectGlobalAssignableDataGroup(queryDTO));
    ...
    protected String getAuthorityTypeCode() {
        return Constants.DocLocalAuthorityTypeCode.DATA_GROUP;
    }
}
