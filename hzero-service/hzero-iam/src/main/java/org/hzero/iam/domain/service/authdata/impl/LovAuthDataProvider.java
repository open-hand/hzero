 ...
 * 值集权限数据提供器
 *
 * @author bo.he02@hand-china.com 2020/05/26 17:05
 */
@Component
public class LovAuthDataProvider extends AbstractAuthDataProvider {

    @Autowired
    public LovAuthDataProvider(AuthDataRepository authDataRepository) {
        super(authDataRepository);
    }

    @Override
    public String getAuthorityTypeCode() {
        return Constants.AUTHORITY_TYPE_CODE.LOV;
    }

    @Override
    public List<AuthDataVo> findAuthData(@Nonnull String authorityTypeCode, @Nonnull AuthDataCondition authDataCondition) {
        return this.singleAuthDataVo(this.getAuthDataRepository().queryLovDataSourceInfo(authDataCondition));
    }
}
