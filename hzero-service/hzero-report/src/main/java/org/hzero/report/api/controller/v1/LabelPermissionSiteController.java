 ...
 */
@Api(tags = ReportSwaggerApiConfig.LABEL_PERMISSION_SITE)
@RestController("labelPermissionSiteController.v1")
@RequestMapping("/v1/label-permissions")
public class LabelPermissionSiteController extends BaseController {
    ...
    @GetMapping
    @CustomPageRequest
    public ResponseEntity<Page<LabelPermission>> list(Long tenantId, @Encrypt Long labelTemplateId,
                                                      @ApiIgnore @SortDefault(value = LabelPermission.FIELD_PERMISSION_ID,
                                                              direction = Sort.Direction.DESC) PageRequest pageRequest) {
        return Results.success(labelPermissionRepository.pageLabelPermission(labelTemplateId, tenantId, false, pageRequest));
    }

    @ApiOperation(value = "创建标签权限")
    @Permission(level = ResourceLevel.SITE)
    @PostMapping
    public ResponseEntity<LabelPermission> create(@Encrypt @RequestBody LabelPermission labelPermission) {
    ...
    @Permission(level = ResourceLevel.SITE)
    @PutMapping
    public ResponseEntity<LabelPermission> update(@Encrypt @RequestBody LabelPermission labelPermission) {
        validObject(labelPermission);
