 ...
 */
@Api(tags = ReportSwaggerApiConfig.TEMPLATE)
@RestController("templateController.v1")
@RequestMapping("/v1/{organizationId}/templates")
public class TemplateController extends BaseController {
    ...
    @GetMapping
    @CustomPageRequest
    public ResponseEntity<Page<Template>> list(@PathVariable Long organizationId, Template template,
                                               @ApiIgnore @SortDefault(value = Template.FIELD_TEMPLATE_ID, direction = Sort.Direction.DESC) PageRequest pageRequest) {
        template.setTenantId(organizationId);
    ...
    @Permission(level = ResourceLevel.ORGANIZATION)
    @PostMapping
    public ResponseEntity<Template> create(@PathVariable Long organizationId, @RequestBody Template template) {
        template.setTenantId(organizationId);
