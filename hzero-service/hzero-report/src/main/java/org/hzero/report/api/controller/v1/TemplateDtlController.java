 ...
 */
@Api(tags = ReportSwaggerApiConfig.REPORT_TEMPLATE_DTL)
@RestController("templateDtlController.v1")
@RequestMapping("/v1/{organizationId}/template-dtls")
public class TemplateDtlController extends BaseController {
    ...
    @Permission(level = ResourceLevel.ORGANIZATION)
    @PostMapping
    public ResponseEntity<TemplateDtl> create(@PathVariable Long organizationId, @Encrypt @RequestBody TemplateDtl templateDtl) {
        // 设置租户
        templateDtl.setTenantId(organizationId);
        // 验证数据
