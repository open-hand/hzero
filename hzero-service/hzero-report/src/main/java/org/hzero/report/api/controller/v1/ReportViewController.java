 ...
 */
@Api(tags = ReportSwaggerApiConfig.REPORT_VIEW)
@Slf4j
@Controller("reportViewController.v1")
...
public class ReportViewController extends BaseController {
    ...
    public void exportReportFile(@PathVariable("reportUuid") String reportUuid,
        ...
        try {
            ...
            log.error("导出报表文件失败", ex);
        }
    }

    @ApiOperation(value = "导出报表文件(内部调用不鉴权)")
    @Permission(permissionWithin = true)
    @GetMapping(value = "/export/{reportUuid}/{outputType}/inside")
    @ResponseBody
    public void exportReportFileInside(@PathVariable("organizationId") Long organizationId, @PathVariable("reportUuid") String reportUuid,
                                       @PathVariable("outputType") String outputType, HttpServletRequest request,
                                       HttpServletResponse response) {
        try {
            reportService.exportReportFileInside(organizationId, reportUuid, outputType, request, response);
        } catch (final Exception ex) {
            log.error("导出报表文件失败", ex);
        }
    }
