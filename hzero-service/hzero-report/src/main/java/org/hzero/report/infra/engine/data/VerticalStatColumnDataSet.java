 ...
 * 纵向展示报表统计列的报表数据集类。
 *
 * @author xianzhi.chen@hand-china.com 2018年10月17日下午8:39:56
 */
public class VerticalStatColumnDataSet extends AbstractReportDataSet implements ReportDataSet {

    public VerticalStatColumnDataSet(MetaDataSet metaDataSet, String layout, String statColumnLayout) {
    ...
    public String getRowKey(ColumnTreeNode rowNode, ColumnTreeNode columnNode) {
        ...
        return StringUtils.equals(this.layout, HrptConstants.LayoutType.HORIZONTAL)
                ? (columnNode.getPath() + rowNodePath)
    ...
    public List<ReportDataColumn> getHeaderLeftFixedColumns() {
        List<ReportDataColumn> columns =
                (StringUtils.equals(this.layout, HrptConstants.LayoutType.HORIZONTAL)) ? this.getDimColumns()
        ...
        leftFixedColumns.add(new ReportDataColumn(
                new MetaDataColumn("stat_column", "统计列", HrptConstants.ColumnType.STATISTICAL)));
    ...
    public ColumnTree getBodyLeftFixedColumnTree() {
        return StringUtils.equals(this.layout, HrptConstants.LayoutType.HORIZONTAL)
                ? this.getHorizontalLayoutLeftFixedColumnTree()
                : this.getVerticalLayoutLeftFixedColumnTree();
    }

    @Override
    public List<ColumnTreeNode> getBodyRightColumnNodes() {
        return StringUtils.equals(this.layout, HrptConstants.LayoutType.HORIZONTAL)
                ? this.getLayoutColumnTree().getLastLevelNodes()
    ...
    public boolean isHideStatColumn() {
        ...
        return this.getEnabledStatColumns().size() == 1
                && (StringUtils.equals(this.layout, HrptConstants.LayoutType.VERTICAL)
                || this.getDimColumnCount() > 0);
    }
