 ...
 */
@Api(tags = MessageSwaggerApiConfig.CALL_MESSAGE_SITE)
@RestController("sendCallSiteController.v1")
@RequestMapping("/v1/messages/call")
public class SendCallSiteController extends BaseController {

    private final CallSendService callSendService;

    @Autowired
    public SendCallSiteController(CallSendService callSendService) {
    ...
    public ResponseEntity<Message> sendMessageWithTemplate(@RequestBody MessageSender messageSender) {
        ...
        validObject(messageSender.setTenantId(BaseConstants.DEFAULT_TENANT_ID), MessageSender.Call.class);
