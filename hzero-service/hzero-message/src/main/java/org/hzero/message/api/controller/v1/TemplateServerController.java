 ...
 */
@Api(tags = MessageSwaggerApiConfig.TEMPLATE_SERVER)
@RestController("templateServerController.v1")
@RequestMapping("/v1/{organizationId}/template-servers")
public class TemplateServerController extends BaseController {

    private final TemplateServerService templateServerService;

    @Autowired
    public TemplateServerController(TemplateServerService templateServerService) {
    ...
    public ResponseEntity<Page<TemplateServer>> pageTemplate(@PathVariable("organizationId") Long organizationId,
        ...
        return Results.success(templateServerService.pageTemplateServer(organizationId, messageCode, messageName, true, pageRequest));
    }
    ...
    @CustomPageRequest
    public ResponseEntity<TemplateServer> getTemplateServer(@PathVariable Long organizationId,
                                                            @Encrypt @PathVariable @ApiParam("消息模板ID") long tempServerId) {
        return Results.success(templateServerService.getTemplateServer(organizationId, tempServerId));
    }

    @ApiOperation("复制消息发送配置")
    @Permission(level = ResourceLevel.ORGANIZATION)
    @GetMapping("/{tempServerId}/copy")
    @ProcessLovValue(targetField = BaseConstants.FIELD_BODY)
    @CustomPageRequest
    public ResponseEntity<TemplateServer> copyTemplateServer(@PathVariable Long organizationId,
                                                             @Encrypt @PathVariable @ApiParam("消息模板ID") long tempServerId) {
        return Results.success(templateServerService.copyTemplateServer(organizationId, tempServerId));
    }
    ...
    @ProcessLovValue(targetField = BaseConstants.FIELD_BODY)
    public ResponseEntity<List<TemplateServerLine>> getTemplateLines(@PathVariable("organizationId") Long organizationId,
                                                                     @RequestParam("messageCode") String messageCode) {
        return Results.success(templateServerService.listTemplateServerLine(organizationId, messageCode));
    }
    ...
    @GetMapping("/detail")
    @ProcessLovValue(targetField = BaseConstants.FIELD_BODY)
    public ResponseEntity<TemplateServer> getTemplateServerDetail(@PathVariable("organizationId") Long organizationId,
                                                                  @RequestParam("messageCode") String messageCode) {
        return Results.success(templateServerService.getTemplateServer(organizationId, messageCode));
    }

    @ApiOperation("新增消息模板账户关联")
    @Permission(level = ResourceLevel.ORGANIZATION)
    @PostMapping
    public ResponseEntity<TemplateServer> createTemplateServer(@PathVariable("organizationId") Long organizationId, @RequestBody TemplateServer templateServer) {
        templateServer.setTenantId(organizationId);
    ...
    public ResponseEntity<TemplateServer> updateTemplateServer(@PathVariable("organizationId") Long organizationId,
                                                               @Encrypt @PathVariable @ApiParam("模板账户关联ID") long tempServerId,
