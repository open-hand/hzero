...
public class DefaultSocketHandler implements SocketHandler {
    ...
    public void afterConnectionEstablished(WebSocketSession session) {
        try {
            ...
            if (SocketSessionUtils.containsAccessToken(session)) {
                // 前端用户连接
                userConnection(session, sessionId);
            } else if (SocketSessionUtils.containsSecretKey(session) && attributeMap.containsKey(WebSocketConstant.Attributes.GROUP)) {
                // 密钥连接
            ...
            logger.debug("webSocket connection failed. message : {}, token : {}", e.getMessage(), SocketSessionUtils.getAccessToken(session));
        }
    }
    ...
    private void userConnection(WebSocketSession webSocketSession, String sessionId) {
        // 获取用户信息
        String token = SocketSessionUtils.getAccessToken(webSocketSession);
    ...
    public void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        beansOfType.forEach((name, bean) -> {
            if (bean.needPrincipal() && SocketSessionUtils.containsAccessToken(session)) {
                String token = SocketSessionUtils.getAccessToken(session);
