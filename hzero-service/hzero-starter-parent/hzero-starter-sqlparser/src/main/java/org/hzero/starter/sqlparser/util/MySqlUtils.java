...
public class MySqlUtils {
    ...
    static Class<?>       connectionImpl = null;
    ...
    public static Long getId(Object conn) {
        ...
        if (connectionImpl == null) {
            if ("com.mysql.jdbc.ConnectionImpl".equals(clazz.getName())) {
                connectionImpl = clazz;
            } else if ("com.mysql.jdbc.Connection".equals(clazz.getName())) { // mysql 5.0.x
                connectionImpl = clazz;
            } else if ("com.mysql.cj.jdbc.ConnectionImpl".equals(clazz.getName())) { // mysql 5.0.x
                connectionImpl = clazz;
            } else if ("com.mysql.jdbc.ConnectionImpl".equals(clazz.getSuperclass().getName())) {
                connectionImpl = clazz.getSuperclass();
            }
        }

        if (connectionImpl == clazz || connectionImpl == clazz.getSuperclass()) {
            try {
                if (method_getId == null && !method_getId_error) {
                    Method method = connectionImpl.getDeclaredMethod("getId");
                    method.setAccessible(true);
                    method_getId = method;
                }
                if (method_getId == null) {
