...
public class Condition implements IDynamicTableName {
    ...
    public Condition(Class<?> entityClass, boolean exists, boolean notNull) {
        ...
        table = EntityHelper.getTableByEntity(entityClass);
        //根据李领北建议修改#159
    ...
    public static class Builder {
        ...
        public Builder(Class<?> entityClass, boolean exists, boolean notNull) {
            ...
            this.table = EntityHelper.getTableByEntity(entityClass);
        ...
        public Condition build() {
            this.conditionCriterias = new ArrayList<Criteria>();
            ...
            if (propertyMap.containsKey(property)) {
                ...
                if (EntityHelper.contain(entityClass)) {
                    ...
                    if (entityTable.isMultiLanguage()) {
                        if (entityTable.getMultiLanguageColumns().stream().map(EntityColumn::getColumn).collect(Collectors.toList()).contains(column)) {
                            column = " t." + column;
                        } else {
                            column = " b." + column;
                        }
    ...
    public Set<String> getSelectColumns() {
        ...
        if (!CollectionUtils.isEmpty(selectColumns)) {
            if (EntityHelper.contain(entityClass)) {
                ...
                if (entityTable.isMultiLanguage()) {
                    Set<String> selectColumnSet = new HashSet<>(selectColumns.size());
                    Set<String> multiLanguageColumns = Optional
                            .ofNullable(entityTable.getMultiLanguageColumns())
                            .orElse(Collections.emptySet())
                            .stream()
                            .map(EntityColumn::getColumn)
                            .collect(Collectors.toSet());
                    Set<String> pkColumns = Optional
                            .ofNullable(entityTable.getEntityClassPkColumns())
                            .orElse(Collections.emptySet())
                            .stream()
                            .map(EntityColumn::getColumn)
                            .collect(Collectors.toSet());
                    for (String selectColumn : selectColumns) {
                        if (multiLanguageColumns.contains(selectColumn)) {
                            selectColumnSet.add("t." + selectColumn);
                        } else if (pkColumns.contains(selectColumn)) {
                            selectColumnSet.add("b." + selectColumn);
                        } else {
                            selectColumnSet.add(selectColumn);
                        }
                    }
