...
public class DefaultLovSqlHandle implements LovSqlHandler {
    ...
    private static final Logger logger = LoggerFactory.getLogger(DefaultLovSqlHandle.class);

    private static final Pattern PATTERN = Pattern.compile(LovConstants.CONTENT_REGEX);
    private static final Pattern NUMBER_PATTERN = Pattern.compile("^[-\\+]?\\d+$");
    private static final String TRANSLATION_PARAMS = "_lovTranslationParams";
    private static final String TRANSLATION_PARAM_FOREACH = "<foreach collection=\"" + TRANSLATION_PARAMS
    ...
    @Override
    public List<Map<String, Object>> queryData(String lovCode, Long tenantId, Map<String, Object> queryParam, int page, int size) {
        // 解密参数
        queryParam = decryptParam(queryParam);
        // check lov code
        Assert.notNull(lovCode, String.format(LovConstants.ErrorMessage.ERROR_NULL, "lov code"));
        // check sql
        LovDTO lov = this.lovAdapter.queryLovInfo(lovCode, tenantId);
        String customSql = this.lovSqlGetter.getCustomSql(lovCode, tenantId);
    ...
    private void checkDollarParams(Map<String, Object> queryParam) {
        if (MapUtils.isEmpty(queryParam)) {
            return;
        }
        for (Map.Entry<String, Object> entry : queryParam.entrySet()) {
            if (entry.getValue() instanceof String) {
                checkDollarParamValue(entry.getKey(), (String) entry.getValue());
            }
        }
    }

    private void checkDollarParamValue(String paramName, String value) {
        LovProperties.Sql sqlProperties = getLovSqlProperties();
        checkDollarParamValue(sqlProperties, paramName, value);
    }

    private void checkDollarParamValue(LovProperties.Sql sqlProperties, String paramName, String value) {
        if (StringUtils.isBlank(value)) {
            return;
        }
        if (containsSqlInjectionRisk(sqlProperties, value)) {
            logger.warn("invalid lov sql parameter value, paramName:{}", paramName);
            throw new CommonException(LovConstants.ErrorMessage.ERROR_INVALIDE_SQL);
        }
    }

    private boolean containsSqlInjectionRisk(LovProperties.Sql sqlProperties, String text) {
        if (CollectionUtils.isNotEmpty(sqlProperties.getDollarParamDenyTokens())) {
            for (String token : sqlProperties.getDollarParamDenyTokens()) {
                if (StringUtils.isNotEmpty(token) && text.contains(token)) {
                    return true;
                }
            }
        }
        if (CollectionUtils.isNotEmpty(sqlProperties.getDollarParamDenyKeywords())) {
            for (String keyword : sqlProperties.getDollarParamDenyKeywords()) {
                if (StringUtils.isNotBlank(keyword)
                        && Pattern.compile(".*\\b" + Pattern.quote(keyword) + "\\b.*", Pattern.CASE_INSENSITIVE).matcher(text).matches()) {
                    return true;
                }
            }
        }
    ...
    private LovProperties.Sql getLovSqlProperties() {
        return ApplicationContextHelper.getContext().getBean(LovProperties.class).getSql();
    }
    ...
    @Override
    public List<Map<String, Object>> queryTranslationData(String lovCode, Long tenantId, List<String> params) {
        return queryTranslationData(lovCode, tenantId, null, params);
    }

    @Override
    public List<Map<String, Object>> queryTranslationData(String lovCode, Long tenantId, Map<String, Object> queryParams, List<String> params) {
        ...
        Assert.notNull(lovCode, String.format(LovConstants.ErrorMessage.ERROR_NULL, "lov code"));
        // check sql
        String translationSql = this.lovSqlGetter.getTranslationSql(lovCode, tenantId);
        ...
        translationSql = translationSql.replace(arg, TRANSLATION_PARAM_FOREACH);
        List<?> translationParams = prepareTranslationParams(params);
        logger.debug("translation sql is :\n {}", translationSql);
        checkDollarParams(queryParams);
        // do query
        try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
            DynamicSqlMapper sqlMapper = new DefaultDynamicSqlMapper(sqlSession);
            queryParams.put(TRANSLATION_PARAMS, translationParams);
            if (DetailsHelper.getUserDetails() != null) {
                queryParams.put("request", DetailsHelper.getUserDetails());
            }
    ...
    private boolean isNumber(List<String> list) {
        return list.stream().allMatch(item -> item != null && NUMBER_PATTERN.matcher(item).matches());
    }

    private List<?> prepareTranslationParams(List<String> params) {
        if (isNumber(params)) {
            return params.stream().map(BigDecimal::new).collect(Collectors.toList());
        }
        return params.stream().map(this::removeOuterQuotes).collect(Collectors.toList());
    }
    ...
    private String removeOuterQuotes(String value) {
        ...
        return start >= end ? StringUtils.EMPTY : value.substring(start, end).replace("''", "'");
    }

    @Override
    public List<Map<String, Object>> getLovSqlMeaning(String lovCode, Long tenantId, Map<String, Object> queryParams) {
