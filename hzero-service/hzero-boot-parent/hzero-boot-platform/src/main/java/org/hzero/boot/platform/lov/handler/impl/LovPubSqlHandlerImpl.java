...
public class LovPubSqlHandlerImpl implements LovPubSqlHandler {
    ...
    public List<Map<String, Object>> queryPubData(String lovCode, String lang, Long tenantId, Map<String, Object> queryParam, int page, int size) {
        ...
        if (BaseConstants.Flag.NO.equals(lov.getMustPageFlag())) {
            size = 0;
        }
    ...
    public List<Map<String, Object>> queryPubTranslationData(String lovCode, String lang, Long tenantId, List<String> params) {
        ...
        if (matcher.find()) {
            arg = matcher.group();
            String paramName = arg.substring(2, arg.length() - 1);
            // 检查是否为decrypt格式：decrypt(param)
            if (paramName.startsWith("decrypt(") && paramName.endsWith(")")) {
                params = params
                        .stream()
                        .map(param -> encryptionService.decrypt(param, ""))
                        .collect(Collectors.toList());
            }
        }

        translationSql = translationSql.replace(arg, TRANSLATION_PARAM_FOREACH);
        List<?> translationParams = isNumber(params) ? params.stream().map(BigDecimal::new).collect(Collectors.toList()) : params;
        logger.debug("translation sql is :\n {}", translationSql);
        // do query
        try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
            DynamicSqlMapper sqlMapper = new DefaultDynamicSqlMapper(sqlSession);
            Map<String, Object> param = new HashMap<>(16);
            param.put(TRANSLATION_PARAMS, translationParams);
