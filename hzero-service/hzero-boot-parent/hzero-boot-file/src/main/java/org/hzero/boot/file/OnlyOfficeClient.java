...
public class OnlyOfficeClient {
        ...
        if (BootFileConstant.EditType.HOFFICE.equals(onlyOfficeConfig.getEditType())) {
            ...
            try {
                return onlyOfficeService.generateHtml(url, tokenUrl, fileName, permission,
                        objectMapper.writeValueAsString(extraDTO), buildCallBackUrl());
            } catch (Exception e) {
    ...
    private String buildCallBackUrl() {
        String callBackUrl = StringUtils.stripEnd(onlyOfficeConfig.getCallBackUrl(), BaseConstants.Symbol.SLASH);
        return callBackUrl + "?x9s7d8=" + TokenUtils.getToken();
    }
