...
public class HeaderWrapperFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (CollectionUtils.isEmpty(token)) {
            LOGGER.info("Request get empty jwt , request uri: {} method: {}", request.getURI().getPath(), request.getMethod());
            return chain.filter(exchange);
        } else {
            if (gatewayHelperProperties.isEnabledJwtLog()) {
                LOGGER.info("Request get jwt , request uri: {} method: {} JWT: {}",
                        request.getURI().getPath(), request.getMethod(), token);
            }
            return chain.filter(
                    exchange.mutate()
                    .request(
                            builder -> builder.header(HEADER_JWT, token.get(0))
                    )
                    .build());
        }
    }
