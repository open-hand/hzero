...
import org.hzero.gateway.util.RedisOpUtils;
import org.hzero.gateway.util.RequestCountUtils;
 ...
 * <p>
 * id
 * key:    hadm:api:monitor:ids
 * value:  1,2
 * <p>
 * pattern
 * key:    hadm:api:monitor:patterns
 * value:  /hadm/**,/hiam/**
 * <p>
 * time_window_size
 * key:    hadm:api:monitor:windows
 * value:  5,5
 *
 * @author XCXCXCXCX
 * @version 1.2.0
...
public final class RequestCountRules {
    ...
    public RequestCountRules(RedisHelper redisHelper, RedisWhiteSetRepository whiteSetRepository, RedisBlackSetRepository blackSetRepository) {
    ...
    protected List<Long> refreshRules() {
        ...
        List<Integer> tmpWindows = RedisOpUtils.selectDbAndClear(redisHelper, RequestCountTraceListener.SELECT_DB,
                () -> redisHelper
                        .lstAll(keyPrefix + TIME_WINDOW_SIZES_KEY)
                        .stream()
                        .map(Integer::parseInt)
                        .collect(Collectors.toList()));
        Map<String, String> tmpMap = RedisOpUtils.selectDbAndClear(redisHelper, RequestCountTraceListener.SELECT_DB,
                () -> redisHelper
                        .hshGetAll(keyPrefix + BLACKLIST_THRESHOLD_KEY));
        List<Set<String>> tmpBlacklist = RedisOpUtils.selectDbAndClear(redisHelper, RequestCountTraceListener.SELECT_DB,
                () -> {
        ...
        List<Set<String>> tmpWhitelist = RedisOpUtils.selectDbAndClear(redisHelper, RequestCountTraceListener.SELECT_DB,
                () -> {
                    if (!CollectionUtils.isEmpty(tmpIds)) {
                        List<Set<String>> tmp = new ArrayList<>();
                        for (Long id : tmpIds) {
